...
@property (nonatomic, strong) UIButton * showSmartPhotoButton;
...
- (void)viewDidLoad {
    ...
    self.showSmartPhotoButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.showSmartPhotoButton.backgroundColor = [UIColor yellowColor];
    self.showSmartPhotoButton.frame = CGRectMake(100, 460, 150, 100);
    [self.allPhotosButton setTitle:@"第二项 图片集" forState:UIControlStateNormal];
    [self.showSmartPhotoButton addTarget:self action:@selector(getAllPhotoAlbumName) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:self.showSmartPhotoButton];
}
...
- (void)getAllPhotoAlbumName
{
    ...
    DJSShowSmartAblumTableViewController * smartAblumTableViewController = [[DJSShowSmartAblumTableViewController alloc] init];
    smartAblumTableViewController.albumNameArr = [NSMutableArray arrayWithArray:nameArr];
    smartAblumTableViewController.albumAssetsArr = [NSMutableArray arrayWithArray:assetArr];
    [self presentViewController:smartAblumTableViewController animated:YES completion:nil];
}
